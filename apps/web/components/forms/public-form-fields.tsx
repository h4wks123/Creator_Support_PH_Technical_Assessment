...
export default function PublicFormFields({ form }: { form: PublicForm }) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    setSubmitting(true);
    try {
      await submitPublicForm(form.slug, email.trim(), form.questions.map((question) => ({ questionId: question.id, value: answers[question.id] ?? null })));
      setSubmitted(true);
    } catch {
  ...
  if (submitted) {
    return <section className="mx-auto max-w-3xl rounded-xl border border-slate-200 bg-white p-8 text-center shadow-sm"><p className="text-xs font-semibold uppercase tracking-[0.18em] text-primary">Response submitted</p><h1 className="mt-3 font-[Poppins] text-2xl font-semibold">Thank you for your response.</h1><p className="mt-2 text-sm text-slate-500">Your answers have been recorded.</p></section>;
  }
