...
export default function ResponseDetail({ formId, responseId }: { formId: string; responseId: string }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const loadedResponse = await getFormResponse(formId, responseId);
        if (!active) return;
